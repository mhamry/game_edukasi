...
document.querySelectorAll(".game").forEach(function (game, index) {
  ...
  buttons.forEach(function (button) {
    button.addEventListener("click", function () {
      ...
      tryagain.pause();
      backsound.pause();
      // const inputValue = value.value.replace(/[^\w\s]/gi, "").toLowerCase();
      // const cleanJawaban = jawaban.replace(/[^\w\s]/gi, "").toLowerCase();

      const inputValue = value.value
        .replace(/[^\w\s]/gi, "")
        .replace(/\s+/g, "")
        .toLowerCase();
      const cleanJawaban = jawaban
        .replace(/[^\w\s]/gi, "")
        .replace(/\s+/g, "")
        .toLowerCase();
      ...
      } else if (inputValue === cleanJawaban) {
